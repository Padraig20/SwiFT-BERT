...
from .linear import LinearLayerReg, LinearLayerClf
...
def load_model(model_name, hparams=None):
    ...
    if hparams.downstream_task_type == "regression":
        if model_name == "bert":
            ...
            net = LinearLayerReg(dims, hparams.target_dim)
        else:
            raise NameError(f"{model_name} is a wrong model name")
    elif hparams.downstream_task_type == "classification":
        if model_name == "bert":
            net = BERT(hparams.target_dim,
                    dims,
                    t,
            ...
            net = LinearLayerClf(dims, hparams.target_dim)
        else:
            raise NameError(f"{model_name} is a wrong model name")
